...
import {currentSchema} from './extraVars.js';
import {forEachObjIndexed} from 'ramda';
import {
    structureAttrs,
    structureSpecialAttrs,
    styleSpecialAttrs,
    styleAttrs,
...
function memoizeOne(converter, target) {
    const cache = convertedPropCache[target];
    if (cache) {
        return cache;
    }

    const result = converter;
    convertedPropCache[target] = result;
    return result;
}
...
function buildOptions(schema) {
    ...
    let ref;
    const refChildren = {};
    const buildExtraOptions = (v, k) => {
        if (v.attributes) {
            children = [];
            chartType = k;
            forEachObjIndexed(buildExtraOptions, v.attributes);
            allOpts.push(
                <TraceTypeSection
                    traceTypes={[k]}
                    name={k + ' - Other Options'}
                    key={k}
                    children={children}
                />
            );
            ...
            if (
                ...
                !(structureSpecialAttrs[chartType] || []).includes(test) &&
                !(styleSpecialAttrs[chartType] || []).includes(test)
            ) {
                if (v.role) {
                    if (ref) {
                        ref += '.' + k;
                    ...
                    refChildren[chartType + ref] = [];
                    forEachObjIndexed(buildExtraOptions, v);
                    if (ref !== k && ref) {
                        refChildren[chartType + ref.replace('.' + k, '')].push(
                            <PlotlySection
                                name={k}
                                key={k}
                                attr={ref + '.' + k}
                                children={refChildren[chartType + ref]}
                            />
                        );
                        ref = ref.replace('.' + k, '');
                    } else {
                ...
                if (v.valType === 'number') {
                    if (ref) {
                        refChildren[chartType + ref].push(
                            <Numeric label={k} attr={ref + '.' + k} key={k} />
                        );
                    } else {
    ...
    forEachObjIndexed(buildExtraOptions, schema.traces);
    return allOpts;
}

const StyleTracesPanel = (props, {localize: _}) => {
    const schema = memoizeOne(currentSchema(), 'schema');
    const allTraceTypes = Object.keys(schema.traces);
    ...
    return (
        <CustomTraceAccordion canGroup>
            ...
            <PlotlySection name={_('Error Bars X')} mode="trace">
                <ErrorBars attr="error_x" />
            </PlotlySection>
            <PlotlySection name={_('Error Bars Y')} mode="trace">
                <ErrorBars attr="error_y" />
            </PlotlySection>
            <PlotlySection name={_('Error Bars Z')} mode="trace">
